...
# 等同于
$ sudo ln -s '/usr/lib/systemd/system/xx.service' '/etc/systemd/system/xxx/xx.service'
```
...
```shell
$ systemctl cat atd.service  # 查看配置文件内容
[Unit]
...
```
配置文件分成几个区块。每个区块的第一行，是用方括号表示的区别名。
每个区块内部都是一些等号连接的键值对。
...
Unit 配置文件的完整字段清单，可以参考[官方文档](https://www.freedesktop.org/software/systemd/man/systemd.unit.html)。
